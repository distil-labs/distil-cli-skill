...
- Expand multi-turn conversations into individual training examples
...
```bash
# Option 1: Directory (expects traces.jsonl, job_description.json, config.yaml/config.json)
...
You must provide either `--data` or all three individual file flags, but not both.
...
4. **Expand multi-turn conversations** — Set `expand_multiturn_conversations: true` (the default) if your traces contain multi-turn conversations and you want each turn to become a separate training example
