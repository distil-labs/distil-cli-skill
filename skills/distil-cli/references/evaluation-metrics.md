...
- `llm-as-a-judge`: LLM verdict (good=1, bad=0) against the reference. `-reference-free` omits
...
**Teacher evaluation.** The teacher score is the ceiling everything downstream distills from.
Review the score and failure patterns with the user, and confirm this quality is acceptable in
production. That is PROCEED. Otherwise iterate on the teacher choice (ITERATE) or re-examine
the task setup (RETHINK).
...
These are gates for judgment, not thresholds to apply mechanically. When `teacher - base` is
