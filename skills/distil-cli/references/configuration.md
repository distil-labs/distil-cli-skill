...
| `mutators` | `[]` | One entry per dimension to vary, each with a `name` and a list of `values` (plain strings). Optional per entry: `method` (`fixed`, `adaptive`), `target_distribution` (`uniform`, `match_seed`, or one weight per value), `description`, and the ADVANCED `beta` for adaptive ones. No mutators are applied by default. See `mutators.md` |
