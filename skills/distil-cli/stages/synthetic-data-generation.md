...
set, a final teacher pass then minimally repairs corrupted or truncated training targets
...
Every generation call is shaped by three inputs, and knowing which one to reach for is most of
iterating well:
...
- `synthetic_data_generation_instructions` says **how to generate the data** — what inputs
  should look like, their formats, domains, register and noise. It touches generation only, so
  it is the safe place to steer the inputs without redefining the task.
...
That difference is the diagnostic. The two constants shift every example the same way; the
mutators decide how the examples are distributed. So when the whole dataset is wrong in the same
way — a format, a misread rule, the wrong register throughout — fix a constant. When the mix is
wrong — a slice missing, or over-represented against what production sees — change the mutator
...
- `generation_in_single_call`, `num_positive_exemplars_per_generation`, and
  `num_unlabelled_exemplars_per_generation` should be lowered when examples are long; each
...
If the task already names patterns, domains or proportions to cover, translate them into mutator
values now; otherwise keep the defaults and revisit after the smoke analysis.
...
2. **Distribution match against the seed examples**: compare generated vs seed data along the
   dimensions relevant to this task, e.g. length (characters; turns for multi-turn), topic
   coverage, style and register, class balance. Pick the dimensions that matter for the task
   and quantify where possible.
3. **Targeted slices actually materialised**: if mutators or the job description asked for a
   ...
   **Only read this axis when the smoke is large enough to answer it.** A run makes
   ...
   and `generation_in_single_call: 4`, just **16 draws**. A grid of more than 16 cells
