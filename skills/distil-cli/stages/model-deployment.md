...
between them: **hosted** on the platform, or **local** vLLM on downloaded weights. No
smoke/full split either way. One directory per trained model.
...
| Credits | `deployments_from_slms_post`, which starts at **0** | none |
...
Fetch the artifacts (the execution backend § Fetch model artifacts; contents:
`../references/deployment.md` § Artifacts), then serve per `../references/deployment.md`
...
answers; record results in `analysis.md`.
