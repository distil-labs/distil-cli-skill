...
- `train.jsonl` := the current test set — the seed that anchors format, style, and
  difficulty;
...
- config.yaml: a small `generation_target` (a fraction of the current test-set size), NEW
  `mutation_topics` naming the uncovered areas (`../references/mutators.md`; a single-item
  list acts as a constant directive), and `match_generated_distribution_to_seed` OFF — the
  whole point is to leave the observed distribution;
...
the inverted directory is a new SeedDataset, so a run spends one of each
(`../references/platform.md` § Credits) — and the review plan (the Step 4
gate). This stage runs at smoke scale by nature; there is no separate full run.
...
candidates are that file minus the seed — **matched on the user message's `content`, not by
comparing whole rows**. The platform normalises rows on storage (it adds an `images` key to
every message and re-serialises), so a structural or string comparison against your input file
...
Caveat to keep in mind: teacher-generated test rows measure agreement with the teacher — a
softer signal than curated examples, acceptable because the teacher is the distillation
ceiling, but one more reason the review gate is mandatory.
...
marker would silently vanish anyway.
...
- **The incumbent student**, if you intend to compare a new model against it — deploy it
  (`model-deployment.md`) and run the new test set through it. There is no route that re-scores
  a finished SLM against a different test set, so this is the way to get a like-for-like
  number. If that is not worth the deployment, say so plainly in the analysis and treat the new
  test set as a fresh baseline with no prior — an honest "no comparison" beats comparing two
  scores measured on different data.
